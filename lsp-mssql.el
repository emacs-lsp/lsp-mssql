...
;; Keywords: data, languages ;; Package-Requires: ((emacs "25.1") (lsp-mode "6.0") (dash "2.14.1") (f "0.20.0") (ht "2.0") (dash-functional "1.2.0")  (lsp-treemacs "0.1"))
...
(defconst lsp-msslq-executable-files
...
(defun lsp-mssql--extract (filename target-dir)
  ...
  (cond
   ...
   (t (error "msslq server cannot extract \"%s\" on platform %s (yet)" filename system-type))))
...
Uses `powershell' on windows and `tar' on Linux to extract the server binary."
  ...
  (let* ((result (cond ((eq system-type 'darwin)  "osx-x64-netcoreapp2.2.tar.gz")
    ...
    (seq-doseq (file lsp-msslq-executable-files)
...
(defun lsp-mssql--connection-complete (workspace params)
...
(defun lsp-mssql--result-set-available (workspace params))

(defun lsp-mssql--message (workspace params)
  (-let [(&hash "message" (&hash "message" "time" "isError" is-error "batchId" batch-id)) params]
    (funcall (if is-error 'lsp--error 'lsp--info) message )))

(defun lsp-mssql--result-set-updated (workspace params)
  )
...
(defun lsp-mssql--result-set-complete (workspace params)
  (-let* ((marker (lsp-mssql-with-result-buffer
          ...
          ((&hash "ownerUri" owner-uri
                  "resultSetSummary"(&hash "specialAction"
                                           "columnInfo" "complete" "rowCount"
                                           row-count "batchId" "id")) params)
          (loaded-index 0)
...
(defun lsp-mssql--batch-complete (workspace params))
...
(defun lsp-mssql--complete (workspace params)
  (lsp-mssql-with-result-buffer))
...
(defun lsp-mssql--connection-changed (_workspace params))
...
(defun lsp-mssql--show-explorer (tree title expand?)
